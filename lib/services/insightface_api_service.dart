...
class InsightFaceApiService {
  /// Set `INSIGHTFACE_API_BASE=https://your.host/api/v1` in `.env` (HTTPS required in release).
  static Uri? _baseUri() {
  ...
  static Future<List<double>?> extractEmbedding(Uint8List photoBytes) async {
    try {
      ...
      if (url == null) {
        if (kDebugMode) debugPrint('InsightFace: INSIGHTFACE_API_BASE not set or invalid in .env');
        return null;
      }
